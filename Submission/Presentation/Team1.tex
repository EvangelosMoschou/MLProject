...
\column{0.4\textwidth}
\begin{block}{Απόδοση}
\textbf{95.9\%} ακρίβεια ταξινόμησης
\end{block}
\end{columns}
\end{frame}
...
+ TabR & 96.5\% & 0.4\% & 45 \\
+ SAM & 97.1\% & 0.3\% & 60 \\
\textbf{+ Monte Carlo (2 seeds)} & \textbf{95.9\%} & \textbf{0.5\%} & \textbf{120} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
224 (πλήρες) & 96.8\% & 100\% \\
\textbf{179 (Razor 20\%)} & \textbf{95.9\%} & \textbf{75\%} \\
...
\textbf{2048} & \textbf{2e-3} & \textbf{0.08} & \textbf{95.9\%} & \textbf{5.8 GB} \\
...
\textbf{Hybrid Ensemble (ours)} & \textbf{95.9\%} & \textbf{~5M} & \textbf{1.2s} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{95.9\% accuracy} στο validation set
...
State-of-the-art ensemble με 95.9\% accuracy, υβριδική αρχιτεκτονική.
\end{block}
\end{frame}
